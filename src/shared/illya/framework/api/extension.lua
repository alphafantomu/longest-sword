...
--[[ 
	Imported from Discordia
		Module has been modified to work with LuaU, instead of the example below by calling the module and it's submodules, instead math, table and string will bind to
		the same libraries in the top level environment with it's modified functions. The address will be different because the submodules are basically represented
		as wrappers for the libraries in the standard environment
...
local e_math, e_table, e_string = math, table, string;
...
setmetatable(math, {__index = function(self, index)
	return e_math[index] or rawget(self, index);
end});

setmetatable(table, {__index = function(self, index)
	return e_table[index] or rawget(self, index);
end});

setmetatable(string, {__index = function(self, index)
	return e_string[index] or rawget(self, index);
end});
